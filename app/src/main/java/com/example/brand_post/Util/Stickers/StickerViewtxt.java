...
public abstract class StickerViewtxt extends FrameLayout {
...
public StickerViewtxt(Context context) {
    super(context);
    init(context);
}

public StickerViewtxt(Context context, AttributeSet attrs) {
    super(context, attrs);
    init(context);
}

public StickerViewtxt(Context context, AttributeSet attrs, int defStyle) {
...
private void init(Context context){
    ...
    this.iv_flip.setImageResource(R.drawable.mirror);
    ...
    int size2 = convertDpToPixel(80, getContext());
    ...
    this.iv_delete.setOnClickListener(new OnClickListener() {
        ...
        public void onClick(View view) {
            if(StickerViewtxt.this.getParent()!=null){
                ViewGroup myCanvas = ((ViewGroup) StickerViewtxt.this.getParent());
                myCanvas.removeView(StickerViewtxt.this);
            }
        }
    });
        ...
        if(view.getTag().equals("DraggableViewGroup")) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    StickerViewtxt.this.setX(StickerViewtxt.this.getX()+offsetX);
                    StickerViewtxt.this.setY(StickerViewtxt.this.getY() + offsetY);
                    move_orgX = event.getRawX();
                    move_orgY = event.getRawY();
                    break;
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    this_orgX = StickerViewtxt.this.getX();
                    this_orgY = StickerViewtxt.this.getY();

                    scale_orgX = event.getRawX();
                    scale_orgY = event.getRawY();
                    scale_orgWidth = StickerViewtxt.this.getLayoutParams().width;
                    scale_orgHeight = StickerViewtxt.this.getLayoutParams().height;

                    rotate_orgX = event.getRawX();
                    rotate_orgY = event.getRawY();

                    centerX = StickerViewtxt.this.getX()+
                            ((View) StickerViewtxt.this.getParent()).getX()+
                            (float) StickerViewtxt.this.getWidth()/5;
                    ...
                    centerY = StickerViewtxt.this.getY()+
                            ((View) StickerViewtxt.this.getParent()).getY()+
                            statusBarHeight+
                            (float) StickerViewtxt.this.getHeight()/2;
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if(length2 > length1
                        ...
                        StickerViewtxt.this.getLayoutParams().width += offset;
                        StickerViewtxt.this.getLayoutParams().height += offset;
                        onScaling(true);
                        //DraggableViewGroup.this.setX((float) (getX() - offset / 2));
                        //DraggableViewGroup.this.setY((float) (getY() - offset / 2));
                    }else if(length2 < length1
                            && (angle_diff < 25 || Math.abs(angle_diff-180)<25)
                            && StickerViewtxt.this.getLayoutParams().width > size/2
                            && StickerViewtxt.this.getLayoutParams().height > size/2) {
                        //scale down
                        double offsetX = Math.abs(event.getRawX() - scale_orgX);
                        double offsetY = Math.abs(event.getRawY() - scale_orgY);
                        double offset = Math.max(offsetX, offsetY);
